...
public class Course {
    ...
    public void enroll(int studentId) {
        ...
        // if (hasWaitingList()) { // ошибка, нужно смотреть наличие мест в курсе
        if (isFullyEnrolled()) {
    ...
    public void unenroll(int studentId) {
        if (waitingList.contains(studentId)) {
            waitingList.remove((Integer) studentId);
            // waitingList.remove(studentId); исправление
        }
        if (enrollList.contains(studentId)) {
            enrollList.remove((Integer) studentId);
            // enrollList.remove(studentId); исправление
            if (hasWaitingList()) {
