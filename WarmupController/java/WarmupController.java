...
public class WarmupController {
    ...
    public void markLaserOn() {
        on = true;
        double laserActiveDuration = laserStopTime - warmupStartTime; // сколько лазер прогревался
        warmupStartTime = timeProvider.currentTimeMillis() / 1000.0;
        double laserInactiveDuration = warmupStartTime - laserStopTime; // сколько лазер был выключен
        if (laserInactiveDuration >= thresholdDuration) { // если прошло более 60 секунд после отключения
    ...
    public double getRemainingTime() {
        if (!on)
            throw new IllegalStateException("Getting remaining time when the laser is off is not allowed");
        return Math.max(0, currentWarmupDuration - (timeProvider.currentTimeMillis() / 1000.0 - warmupStartTime));
    }

    public WarmupController(TimeProvider timeProvider) {
        // конструктор с фейковым временем
    ...
    public WarmupController() {
        // конструктор с системным временем
